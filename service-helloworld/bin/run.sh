...
MainClass="../resource/service-helloworld-1.0.jar --spring.profiles.active=proc"
#JVM参数配置
...
#自定义日志路径
Log="../logs/service-helloworld.log"
...
function start()
{
    ...
    if [[ "$pid" != "" ]]; then
        ...
        sleep 2
		echo "java -Dappliction=${Tag} ${JVM} -jar ${MainClass}  > ${Empty} 2>&1 &"
    fi
}
function stop() {
    pid=$(ps -ef | grep -v 'grep' | egrep $Tag| awk '{printf $2 " "}')
